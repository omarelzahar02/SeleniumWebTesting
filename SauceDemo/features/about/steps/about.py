...
def step_then_logout_visible(context):
    about_button = WebDriverWait(driver, DELAY_TIME).until(EC.presence_of_element_located((By.ID, "about_sidebar_link")))
    assert about_button.is_displayed(), "About button is not visible"
    thread.sleep(DELAY_TIME)

@when('I click on the about button')
def step_when_user_clicks_logout(context):
    WebDriverWait(driver, DELAY_TIME).until(EC.presence_of_element_located((By.ID, "about_sidebar_link"))).click()
    thread.sleep(DELAY_TIME)

@then('I am redirected to the about page')
def step_then_user_logged_out(context):
    assert 'https://saucelabs.com/' == driver.current_url, "About page visible"